...
void Position_Controller::send_vision_position_estimate(float x, float y, float z, float yaw)
{
...
void Position_Controller::send_set_position_target_local_ned(float x, float y, float z, float yaw)
{
...
void Position_Controller::send_set_attitude_target(float pitch_target, float roll_target, float yaw_target, float ratio_of_vel_z_to_default_speed_up, float yaw_rate, bool use_yaw_rate)
{
...
void Position_Controller::toggle_offboard_control(bool flag)
{
	...
	com.param1 = MAV_MODE_FLAG_CUSTOM_MODE_ENABLED; // flag >0.5 => start, <0.5 => stop
	com.param2 = flag ? 4 : 0;
...
void Position_Controller::enable_attitude_messages(int period)
{
	enable_message_stream(MAVLINK_MSG_ID_ATTITUDE, period);
}

void Position_Controller::enable_local_position_estimate_messages(int period)
{
	enable_message_stream(MAVLINK_MSG_ID_LOCAL_POSITION_NED, period);
}

void Position_Controller::enable_message_stream(int id, int period)
{
	{
		// Prepare command for off-board mode
		mavlink_command_long_t com = {0};
